...
add_shortcode( 'ocm_invoice_data', 'ocm_sh_invoice' );

function ocm_sh_invoice( $atts ) {
    global $current_user;
    ...
    $invoices_all = get_posts( array( 'post_type' => 'ocm_invoice'
                                , 'numberposts' => '-1'
                                , 'meta_key' => '_memberid'
    ...
    if ( count( $invoices ) > 0 ) {
        ...
        foreach ( $invoices as $invoice ) {
			if ($header == 'S') {
				...
				$line1name = __('Amount', TEXT_DOMAIN);
				$line1text = get_post_meta( $invoice->ID , '_amount' , true );
				$line2name = __('Due date', TEXT_DOMAIN);
				...
				$line1name = __('Amount', TEXT_DOMAIN);
				$line1text = get_post_meta( $invoice->ID , '_amount' , true );
				$line2name = __('Service', TEXT_DOMAIN);
