<?php 
    ...
    if ( $ocm_invoice_export_hidden == 'yes' ) {
        ...
        header("Content-Disposition: attachment; filename=" . __('Invoices', TEXT_DOMAIN) . ".csv");
...
<form name="ocm_invoice_export_form" method="post" action="<?php echo admin_url('admin.php').'?page='.$_GET['page'].'&noheader=true'; ?>">
    <div class="metabox-holder ">
        <div class="postbox"> 
            <h3><?php _e('Export Invoices', TEXT_DOMAIN) ?></h3>
        </div>
    </div>
<?php
    echo apply_filters( 'ocm_invoices_export_selection', "" );
?>        
    <input type="submit" name="submit_invoice_export" class="button-primary" value="<?php _e('Export Invoices to File', TEXT_DOMAIN)?>" />
