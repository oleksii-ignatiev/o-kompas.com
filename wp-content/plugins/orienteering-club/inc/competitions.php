...
function ocm_comp_custom_post_init(){
    ...
    $labels = array(
        'name'              => __('Competition Categories', TEXT_DOMAIN),
        'singular_name'     => __('Competition Category', TEXT_DOMAIN),
        'search_items'      => __('Search Competition Categories', TEXT_DOMAIN),
        'all_items'         => __('All Competition Categories', TEXT_DOMAIN),
        'parent_item'       => __('Parent Competition Category', TEXT_DOMAIN),
        'parent_item_colon' => __('Parent Competition Category:', TEXT_DOMAIN),
        'edit_item'         => __('Edit Competition Category', TEXT_DOMAIN), 
        'update_item'       => __('Update Competition Category', TEXT_DOMAIN),
        'add_new_item'      => __('Add New Competition Category', TEXT_DOMAIN),
        'new_item_name'     => __('New Competition Category', TEXT_DOMAIN),
        'menu_name'         => __('Competition Categories', TEXT_DOMAIN),
    ); 	
    ...
    $labels = array(
        'name' => __('Competitions', TEXT_DOMAIN),
        'singular_name' => __('Competition', TEXT_DOMAIN),
        'add_new' => __('Add New Competition', TEXT_DOMAIN),
        'add_new_item' => __('Add New Competition', TEXT_DOMAIN),
        'edit_item' => __('Edit Competition', TEXT_DOMAIN),
        'new_item' => __('New Competition', TEXT_DOMAIN),
        'view_item' => __('View Competition', TEXT_DOMAIN),
        'search_items' => __('Search Competitions', TEXT_DOMAIN),
        'not_found' => __('No Competitions found', TEXT_DOMAIN),
        'not_found_in_trash' => __('No Competitions found in Trash', TEXT_DOMAIN),
        'parent_item_colon' => __('Parent Competition:', TEXT_DOMAIN),
...
add_action('save_post', 'ocm_comp_save_fields', 20, 3);
...
function ocm_comp_add_fields_box() {
    ...
    add_meta_box('ocm_comp_fields_box_id', __('Competition Data', TEXT_DOMAIN), 'ocm_comp_display_fields', 'ocm_comp');
    if ($pagenow == 'post.php') {
		add_meta_box('ocm_comp_ranking_id', __('Ranking', TEXT_DOMAIN), 'ocm_comp_display_ranking', 'ocm_comp');
		add_meta_box('ocm_comp_matches_id', __('Matches', TEXT_DOMAIN), 'ocm_comp_display_matches', 'ocm_comp');
        add_meta_box('ocm_comp_shortcodes_box_id', __('Shortcodes', TEXT_DOMAIN), 'ocm_comp_list_shortcodes', 'ocm_comp', 'side', 'high');
	}
}
function ocm_comp_display_fields() {
    ...
    $groupids = isset($values['_groupid']) ? unserialize($values['_groupid'][0]) : array();
    // print_r($groupids);
    ...
    foreach ($groups as $group) {
        $group_options .= "<input type='checkbox' class='groupid' name='groupid[]' ";  
        $group_options .= in_array($group->ID, $groupids) ? "checked='checked' " : "";           
        $group_options .= "value='$group->ID' />" . get_the_title($group->ID);
    }
    
    $html =  "<tr>\n"
            ."<td align='right'><label>".__('Competition format', TEXT_DOMAIN)."</label></td>\n"
            ...
            ."<tr>\n"
            ."<td align='right'><label>".__('Competition groups', TEXT_DOMAIN)."</label></td>\n"
            ."<td><fieldset>". $group_options ."</fieldset></td>\n"
            ."</tr>\n";
    echo "<table>\n" . $html . "</table>\n";
}
function ocm_comp_display_ranking() {
    ...
    echo "[<a href='".admin_url( '/post-new.php?post_type=ocm_competitor', __FILE__ )."'>".__('Add New Competitors', TEXT_DOMAIN)."</a>]";    
...
function ocm_comp_display_matches() {
    ...
    echo "[<a href='".admin_url( '/post-new.php?post_type=ocm_match', __FILE__ )."'>".__('Add New Matches', TEXT_DOMAIN)."</a>]";    
    echo "<table>";
...
function ocm_comp_save_fields($post_id, $post, $update) {
    global $post;
    ...
    if (($formatid == $_POST['formatid']) && ($formatok == 'false')) {  
        ...
        remove_action('save_post', 'ocm_comp_save_fields', 20, 3);
    ...
    $new_values = $_POST["groupid"] ?: array();
    update_post_meta( $post_id, '_groupid', $new_values );
}
...
function ocm_comp_columns($defaults) {  
    ...
    // new column titles
    $defaults['group']  = __('Competition group', TEXT_DOMAIN);
    $defaults['format']  = __('Competition format', TEXT_DOMAIN);
...
function ocm_competition_list_header( $comp_id ) {
    ...
    $names = array( __("Competition Group", TEXT_DOMAIN), __("Competition", TEXT_DOMAIN),
