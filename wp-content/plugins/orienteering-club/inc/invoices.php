...
function ocm_invoice_custom_post_init(){
    ...
    $labels = array(
        'name' => __('Invoice Categories', TEXT_DOMAIN),
        'singular_name' => __('Invoice Category', TEXT_DOMAIN),
        'search_items' => __('Search Invoice Categories', TEXT_DOMAIN),
        'all_items' => __('All Invoice Categories', TEXT_DOMAIN),
        'parent_item' => __('Parent Invoice Category', TEXT_DOMAIN),
        'parent_item_colon' => __('Parent Invoice Category:', TEXT_DOMAIN),
        'edit_item' => __('Edit Invoice Category', TEXT_DOMAIN), 
        'update_item' => __('Update Invoice Category', TEXT_DOMAIN),
        'add_new_item' => __('Add New Invoice Category', TEXT_DOMAIN),
        'new_item_name' => __('New Invoice Category', TEXT_DOMAIN),
        'menu_name' => __('Invoice Categories', TEXT_DOMAIN),
    ); 	
    ...
    $labels = array(
        'name' => __('Invoices', TEXT_DOMAIN),
        'singular_name' => __('Invoice', TEXT_DOMAIN),
        'add_new' => __('Add New Invoice', TEXT_DOMAIN),
        ...
        'search_items' => __('Search Invoices', TEXT_DOMAIN),
        'not_found' => __('No Invoices found', TEXT_DOMAIN),
        'not_found_in_trash' => __('No Invoices found in Trash', TEXT_DOMAIN),
...
function ocm_invoice_add_fields_box() {
    ...
    add_meta_box('ocm_invoice_fields_box_id', __('Invoice Data', TEXT_DOMAIN), 'ocm_invoice_display_fields', 'ocm_invoice');
	if ($pagenow == 'post-new.php') {
		add_meta_box('ocm_invoice_bulk_box_id', __('Add invoices for members in selected category', TEXT_DOMAIN), 'ocm_invoice_bulk', 'ocm_invoice');
	}
    if ($pagenow == 'post.php') {
        add_meta_box('ocm_invoice_member_box_id', __('Member Data', TEXT_DOMAIN), 'ocm_invoice_member_data', 'ocm_invoice');
    }
}
function ocm_invoice_display_fields() {
    ...
    $html = "
        <tr><td align='right'><label>".__('Member', TEXT_DOMAIN)."</label></td><td><select name='memberid'>\n". $member_options ."</select></td></tr>
        <tr><td align='right'><label>".__('Date of issue', TEXT_DOMAIN)."</label></td><td><input id='issuedate' class='ocm_datepicker' type='text' name='issuedate' value='$issuedate' /></td></tr>
        <tr><td align='right'><label>".__('Due date', TEXT_DOMAIN)."</label></td><td><input id='duedate' class='ocm_datepicker' type='text' name='duedate' value='$duedate' /></td></tr>
        <tr><td align='right'><label>".__('Date of payment', TEXT_DOMAIN)."</label></td><td><input id='paymentdate' class='ocm_datepicker' type='text' name='paymentdate' value='$paymentdate' /></td></tr>
        <tr><td align='right'><label>".__('Service', TEXT_DOMAIN)."</label></td><td><input id='service' type='text' name='service' value='$service' /></td></tr>
        <tr><td align='right'><label>".__('Amount', TEXT_DOMAIN)."</label></td><td><input id='amount' type='text' name='amount' value='$amount' /></td></tr>
        <tr><td align='right'><label>".__('C/D', TEXT_DOMAIN)."</label></td><td>
            <input type='radio' name='creditdebet' id='credit' value='credit' ". $credit_checked ."/> ".__('Credit', TEXT_DOMAIN)."
            <input type='radio' name='creditdebet' id='debet' value='debet' ". $debet_checked ."/> ".__('Debet', TEXT_DOMAIN)."</td>
        </tr>
...
function ocm_invoice_bulk() {
	...
	$description = array( __('Define entries', TEXT_DOMAIN)
	...
	$html_step1 = "
		...
		<table>
		<tr>
		<td><label>".__('Member Category', TEXT_DOMAIN)."</label></td>
		<td>
		".wp_dropdown_categories(array(
					'show_option_none' => __('select category from list...', TEXT_DOMAIN),
		...
		<tr>
		<td><label>".__('Invoice prefix', TEXT_DOMAIN)."</label></td>
		<td><input id='prefix' type='text'></td>
		<td>".__('Invoice name is prefix.date-of-issue.####', TEXT_DOMAIN)."</td>
		</tr>
		</table>
		</p>";
...
function ocm_invoice_member_data() {
    ...
    $html .= sprintf("<tr><td>%s</td><td>%s</td></tr>\n", __('Phone', TEXT_DOMAIN), $member_values['_phone'][0]);
    $html .= sprintf("<tr><td>%s</td><td>%s</td></tr>\n", __('Cell', TEXT_DOMAIN), $member_values['_cell'][0]);
...
function ocm_invoice_columns($defaults) {  
    ...
    $defaults['duedate']  = __('Due Date', TEXT_DOMAIN);
    $defaults['paymentdate']  = __('Date of Payment', TEXT_DOMAIN);
    $defaults['amountservice']  = __('Amount', TEXT_DOMAIN).' / '.__('Description', TEXT_DOMAIN);
    $defaults['category']  = __('Category', TEXT_DOMAIN);
...
function ocm_invoice_filtered_by_category() {
    ...
    if ($typenow == $post_type) {
        ...
        wp_dropdown_categories(array(
            'show_option_all' => __("Show All {$info_taxonomy->label}", TEXT_DOMAIN),
            'taxonomy' => $taxonomy,
            'name' => $taxonomy,
            'orderby' => 'name',
            'selected' => $selected,
...
function ocm_invoice_list_member() { 
    add_meta_box('ocm_member_invoices_id', __('Invoices', TEXT_DOMAIN), 'ocm_member_display_invoices', 'ocm_member');
}
function ocm_member_display_invoices($member) {
    ...
    if ( count( $invoices ) == 0 ) {
        echo __('none', TEXT_DOMAIN);
    } else {
        echo "<table>";
        echo "<tr><td>".__('Date of issue', TEXT_DOMAIN)."</td><td>".__('Due date', TEXT_DOMAIN)."</td><td>".__('Date of payment', TEXT_DOMAIN)."</td><td>".__('Amount', TEXT_DOMAIN)."</td><td>".__('Service', TEXT_DOMAIN)."</td></tr>\n";
        foreach ( $invoices as $invoice ) {
            if ( 'draft' == get_post_status( $invoice->ID ) ) {
                $amount = __('none', TEXT_DOMAIN);
            } else {
...
function ocm_invoice_register_rest_fields(){
	...
	register_rest_field( 'ocm_invoice', 'issue_date', array( 'update_callback' => 'ocm_invoice_update_memberid' 
														  , 'schema' => array( 'description' => __('issue date', TEXT_DOMAIN), 'type' => 'date', 'required' => 'true' )
	) ); 
	register_rest_field( 'ocm_invoice', 'due_date', array( 'update_callback' => 'ocm_invoice_update_memberid' 
														, 'schema' => array( 'description' => __('due date', TEXT_DOMAIN), 'type' => 'date', 'required' => 'true' )
	) ); 
	register_rest_field( 'ocm_invoice', 'amount', array( 'update_callback' => 'ocm_invoice_update_memberid' 
													   , 'schema' => array( 'description' => __('amount', TEXT_DOMAIN), 'type' => 'number', 'required' => 'true' )
	) ); 
	register_rest_field( 'ocm_invoice', 'service', array( 'update_callback' => 'ocm_invoice_update_memberid' 
														, 'schema' => array( 'description' => __('service', TEXT_DOMAIN), 'type' => 'string', 'required' => 'true' )
	) ); 
	register_rest_field( 'ocm_invoice', 'credit_debet', array( 'update_callback' => 'ocm_invoice_update_memberid' 
															, 'schema' => array( 'description' => __('credit/debet', TEXT_DOMAIN), 'type' => 'string', 'required' => 'true' )
	) ); 
}
...
function ocm_invoice_list_header() {
    ...
    $names = array( __("issue date", TEXT_DOMAIN), __("due date", TEXT_DOMAIN), __("payment date", TEXT_DOMAIN),
                    __("service", TEXT_DOMAIN),
                    __("amount", TEXT_DOMAIN), __("credit/debet", TEXT_DOMAIN),
