...
function ocm_match_custom_post_init(){
    ...
    $labels = array(
        'name' => __('Matches', TEXT_DOMAIN),
        'singular_name' => __('Match', TEXT_DOMAIN),
        'add_new' => __('Add New Match', TEXT_DOMAIN),
        'add_new_item' => __('Add New Match', TEXT_DOMAIN),
        'edit_item' => __('Edit Match', TEXT_DOMAIN),
        'new_item' => __('New Match', TEXT_DOMAIN),
        'view_item' => __('View Match', TEXT_DOMAIN),
        'search_items' => __('Search Matches', TEXT_DOMAIN),
        'not_found' => __('No Matches found', TEXT_DOMAIN),
        'not_found_in_trash' => __('No Matches found in Trash', TEXT_DOMAIN),
        'parent_item_colon' => __('Parent Match:', TEXT_DOMAIN),
        'menu_name' => __('Matches', TEXT_DOMAIN),
    );
    $args = array(
        'labels' => $labels,
        'hierarchical' => false,
...
function ocm_match_add_fields_box() {
    ...
    add_meta_box('ocm_match_fields_box_id', __('Match Data', TEXT_DOMAIN), 'ocm_match_display_fields', 'ocm_match');
	if ($pagenow == 'post-new.php') {
		add_meta_box('ocm_match_bulk_box_id', __('Add matches for selected competition', TEXT_DOMAIN), 'ocm_match_bulk', 'ocm_match');
	}
    if ($pagenow == 'post.php') {
        add_meta_box('ocm_match_shortcodes_box_id', __('Shortcodes', TEXT_DOMAIN), 'ocm_match_list_shortcodes', 'ocm_match', 'side', 'high');
    }
}
function ocm_match_display_fields() {
    ...
    $html = "  
        <tr><td align='right'><label>".__('Competitors', TEXT_DOMAIN)."</label></td>
		...
		. apply_filters( 'ocm_match_display_single_competitor_field'
		               , __('vs.', TEXT_DOMAIN) ." <select name='compid_2'>\n". apply_filters( 'ocm_match_display_competitors_field', $competitor_2_options, $competitionid) ."</select>"
					   , $competitionid
					   ) ."
		</td>
        </tr>
        <tr><td align='right'><label>".__('Result', TEXT_DOMAIN)."</label></td>
        ...
        <tr><td align='right'><label>".__('Time', TEXT_DOMAIN)."</label></td><td><input id='time' type='text' name='time' value='$time' /></td></tr>
        <tr><td align='right'><label>".__('Competition', TEXT_DOMAIN)."</label></td><td><select id='competitionid'>\n". $comp_options ."</select></td></tr>"
...
function ocm_match_bulk() {
	...
	$html_step1 = "
		...
		<table>
		<tr>
		<th><label>".__('Competition format', TEXT_DOMAIN)."</label></th>
		</tr>
		<tr>
		<td><input type='radio' name='competition_format' id='league_format' value='league' selected='selected'/>" . __('League', TEXT_DOMAIN) . "</td>
		<td><label>".__('Number of matches vs. each competitor', TEXT_DOMAIN)."</label></td>
		<td><input id='bulk_nrvsteam' type='text' /></td>
		</tr>
		<tr>
		<td><input type='radio' name='competition_format' id='knockout_format' value='knockout' />" . __('Knockout', TEXT_DOMAIN) . "</td>
		</tr>
		<tr>
		<td><input type='radio' name='competition_format' id='individual_format' value='individual' />" . __('Individual', TEXT_DOMAIN) . "</td>
		</tr>
		<tr>
		<td><input type='radio' name='competition_format' id='ladder_format' value='ladder' />" . __('Ladder', TEXT_DOMAIN) . "</td>
		</tr>
		</table>
		</p>";
...
function ocm_match_columns($defaults) {  
    ...
    $defaults['match']  = __('Competitors', TEXT_DOMAIN);
    $defaults['result']  = __('Result', TEXT_DOMAIN);
    $defaults['datetime']  = __('Date', TEXT_DOMAIN)." (".__('Time', TEXT_DOMAIN).")";
...
function ocm_match_list_header( $comp_id ) {
    ...
    $names = array( __("Date", TEXT_DOMAIN), __("Time", TEXT_DOMAIN),
                    __("Competitor 1", TEXT_DOMAIN), __("Competitor 2", TEXT_DOMAIN),
...
function ocm_match_list_entry($match_id) {
	...
	$compgroup = ( $compgroupid != '' ? get_the_title( $compgroupid ) : __('none', TEXT_DOMAIN) );
