...
function ocm_competitor_custom_post_init(){
    
    // custom post type: Competitor
    $labels = array(
        'name' => __('Competitors', 'o-kompas'),
        'singular_name' => __('Competitor', 'o-kompas'),
        'add_new' => __('Add New Competitor', 'o-kompas'),
        'add_new_item' => __('Add New Competitor', 'o-kompas'),
        'edit_item' => __('Edit Competitor', 'o-kompas'),
        'new_item' => __('New Competitor', 'o-kompas'),
        'view_item' => __('View Competitor', 'o-kompas'),
        'search_items' => __('Search Competitors', 'o-kompas'),
        'not_found' => __('No Competitors found', 'o-kompas'),
        'not_found_in_trash' => __('No Competitors found in Trash', 'o-kompas'),
        'parent_item_colon' => __('Parent Competitor:', 'o-kompas'),
        'menu_name' => __('Competitors', 'o-kompas'),
    );
    $args = array(
        'labels' => $labels,
        'hierarchical' => false,
    ...
    register_post_type('ocm_competitor', $args);
}

// hooks and filters for Competitor
add_action('add_meta_boxes', 'ocm_competitor_add_fields_box');
add_action('save_post', 'ocm_competitor_save_fields');
add_filter('manage_ocm_competitor_posts_columns', 'ocm_competitor_columns');  
add_filter('manage_edit-ocm_competitor_sortable_columns', 'ocm_competitor_sortable_column');  
add_action('manage_ocm_competitor_posts_custom_column', 'ocm_competitor_column_content', 10, 2);   
add_action('rest_api_init', 'ocm_competitor_register_rest_fields');

// comp fields operations

function ocm_competitor_add_fields_box() {
    global $pagenow, $typenow;
	
    add_meta_box('ocm_competitor_fields_box_id', __('Competitor Data', 'o-kompas'), 'ocm_competitor_display_fields', 'ocm_competitor');
	if ($pagenow == 'post-new.php') {
		add_meta_box('ocm_competitor_bulk_box_id', __('Add competitors for members in selected category', 'o-kompas'), 'ocm_competitor_bulk', 'ocm_competitor');
	}
    if ($pagenow == 'post.php') {
		add_meta_box('ocm_teamplayer_list_box_id', __('Team Players', 'o-kompas'), 'ocm_competitor_display_team', 'ocm_competitor');
        add_meta_box('ocm_competitor_shortcodes_box_id', __('Shortcodes', 'o-kompas'), 'ocm_competitor_list_shortcodes', 'ocm_competitor', 'side', 'high');
	}
}
function ocm_competitor_display_fields() {
    global $post;
    ...
    wp_nonce_field('ocm_competitor_frm_nonce', 'ocm_competitor_frm_nonce');
...
function ocm_competitor_bulk() {
	
	$description = array( 
        __('Define entries', 'o-kompas'),
...
function ocm_competitor_display_team() {
    global $post;
...
function ocm_competitor_list_shortcodes() {
	global $post;
	
	$html = __('Use the following shortcodes by copy/pasting into a post or page (see documentation).', 'o-kompas');

	$html .= sprintf("<p>[ocm_team_data competitor_id='%s']</p>", $post->ID);
	$html .= sprintf("<p>[ocm_match_data competitor_id='%s']</p>", $post->ID);
	
	echo $html;
}

function ocm_competitor_save_fields($post_id) {
    // Bail if we're doing an auto save
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE)
        return;

        // if our nonce isn't there, or we can't verify it, bail
    if (!isset($_POST['ocm_competitor_frm_nonce']) || !wp_verify_nonce($_POST['ocm_competitor_frm_nonce'], 'ocm_competitor_frm_nonce'))
        return;
	...
	do_action( 'ocm_competitor_save_format_fields', $post_id, $competition_id );     
}

// display comp data on admin page

function ocm_competitor_columns($defaults) {  
    // no author, (post)date
    ...
    $defaults['disqualified']  = __('Disqualified', 'o-kompas');
    $defaults['competition']  = __('Competition', 'o-kompas');
    
    return $defaults;  
}  
  
function ocm_competitor_sortable_column( $columns ) {  
    $columns['name'] = 'name';  
...
function ocm_competitor_column_content($column_name, $post_ID) {  
    switch($column_name){
        case 'competition':
...
// rest api
function ocm_competitor_register_rest_fields(){
	register_rest_field( 'ocm_competitor', 'name', array( /*'get_callback' => 'ocm_member_get_name' 
														, */'update_callback' => 'ocm_competitor_update' 
														, 'schema' => array( 'description' => __('Name', 'o-kompas') , 'type' => 'string', 'required' => 'false' )) ); 
	register_rest_field( 'ocm_competitor', 'member_id', array( /*'get_callback' => 'ocm_member_get_middle_name' 
															 , */'update_callback' => 'ocm_competitor_update' 
															 , 'schema' => array( 'description' => 'ocm_member post id' , 'type' => 'integer', 'required' => 'false' )) ); 
	register_rest_field( 'ocm_competitor', 'competitor_type', array( /*'get_callback' => 'ocm_member_get_first_name' 
																   , */'update_callback' => 'ocm_competitor_update' 
																   , 'schema' => array( 'description' => __('Competitor type', 'o-kompas') , 'type' => 'string', 'required' => 'true' )) ); 
	register_rest_field( 'ocm_competitor', 'competition_id', array( 'get_callback' => 'ocm_competitor_get_competition_id' 
																  , 'update_callback' => 'ocm_competitor_update' 
...
function ocm_competitor_get_competition_id($post) { return get_post_meta( $post['id'], '_competitionid', true ); }

function ocm_competitor_update($value, $post, $key) { 
	switch ( $key ) {
