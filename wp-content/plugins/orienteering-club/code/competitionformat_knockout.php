...
function ocm_comp_format_knockout_get_format( $html, $id ) {
    ...
    $html .= "value=" . OCM_CFMT_KNOCKOUT . ">" . __('Knockout', 'o-kompas') . "</option>\n";
...
function ocm_comp_format_knockout_display_format( $html, $compid ) {
    ...
    if ( get_post_meta( $compid, '_formatid', true) == OCM_CFMT_KNOCKOUT ) {
        $html .= sprintf("%s [%s]", __('Knockout', 'o-kompas'), get_post_meta( $compid , '_size' , true )); 
    }
...
function ocm_comp_format_knockout_display_format_fields( $html, $compid ) {
    ...
    $html .= "<tr>\n";
    $html .= "<td align='right'><label>" . __('Scheme size', 'o-kompas') . "</label></td>";
    if ($formatok == 'false') {
...
function ocm_comp_format_knockout_display_ranking( $html, $compid, $display_all, $display_header ) {
    ...
    if ( $display_header == true ) {
		if ( is_admin() ) {
			$html .= "<tr><th>".__('Competition', 'o-kompas')."</th><th>".__('Rank', 'o-kompas')."</th><th>".__('Competitor', 'o-kompas')."</th></tr>\n";
		} else {
			$html .= "<div id=\"scm_ranking\" class=\"header knockout\">\n"; 
			$html .= "<div id=\"scm_ranking_competition\" class=\"header knockout\">" . __('Competition', 'o-kompas') . "</div>\n"; 
			$html .= "<div id=\"scm_ranking_rank\" class=\"header knockout\">" . __('Rank', 'o-kompas') . "</div>\n"; 
			...
			if ( '' == $result ) {
				if (($compid_1 != '') || ($compid_2 != '')) {
					if ( is_admin() ) {
						$string = sprintf("<td>%s</td><td>%s - %s</td>"
					                 , ($k == $firstvalid ? "1/".$cnt." ".__('finalist', 'o-kompas') : "")
									 , ( $compid_1 != '' 
									   ? ( sprintf("<a href=%s target=\"_blank\">%s</a>", site_url( "/wp-admin/post.php?post=$compid_1&action=edit" ), get_the_title($compid_1)))
									   : ( ($results[2 * $h] == '') ? sprintf("%s #%s", __('Winner', 'o-kompas'), (2 * $h) ) : $results[2 * $h])
									   )
									 , ( $compid_2 != '' 
									   ? ( sprintf("<a href=%s target=\"_blank\">%s</a>", site_url( "/wp-admin/post.php?post=$compid_2&action=edit" ), get_the_title($compid_2)))
									   : ( ($results[1 + 2 * $h] == '') ? sprintf("%s #%s", __('Winner', 'o-kompas'), (1 + 2 * $h) ) : $results[1 + 2 * $h])
									   )
									 );
					} else {
						$string = "<div id=\"scm_ranking_rank\" class=\"knockout\">" . ($k == $firstvalid ? "1/".$cnt." ".__('finalist', 'o-kompas') : "&nbsp") . "</div>\n"; 
						$string .= "<div id=\"scm_ranking_competitor\" class=\"knockout\">";
						$string .= sprintf("%s ". __('vs', 'o-kompas') ." %s"
				...
				if ($cnt == 1) {
					if ( is_admin() ) {
						...
						$string  = "<div id=\"scm_ranking_rank\" class=\"knockout\">" . __('Winner', 'o-kompas') . "</div>\n"; 
...
function ocm_comp_format_knockout_display_empty_competitor( $predecessor, $matches, $compid) {
	...
	$html = __('bye', 'o-kompas');
	if (($predecessor < $size) && ( $results[$predecessor] == '')) {
		$html = sprintf("%s #%s", __('Winner', 'o-kompas'), $predecessor );
	}	
					...
					if (((($r/$k) % (4*$k)) % 4) == 0) {
						...
						foreach ( $matches as $match ) {
							if ( $match_number == get_post_meta( $match->ID, '_number', true ) ) {
								...
								$cell = ( $competitor_1 != "" ? get_the_title( $competitor_1 ) : __('bye', 'o-kompas') );
							}
						}
					} else if (((($r/$k) % (4*$k)) % 4) == 2) {
						$match_number = intval(($r + 2*$size)/(4*$k));
						...
						foreach ( $matches as $match ) {
							if ( $match_number == get_post_meta( $match->ID, '_number', true ) ) {
								...
								$cell = ( $competitor_2 != "" ? get_the_title( $competitor_2 ) : __('bye', 'o-kompas') );
							}
						}
					} 			
				} else {
...
function ocm_comp_format_knockout_display_competitors_field( $html, $compid ) {
	...
	return "<option value=''> ".__('unknown', 'o-kompas')." </option>\n" . $html;
}
...
function ocm_comp_format_knockout_display_format_match_fields( $html, $matchid, $compid ) {
	...
	$html  = "<tr><td align='right'><label>".__('Number #', 'o-kompas')."</label></td><td><input id='number' type='text' name='number' value='$number' /></td></tr>";
	$html .= "<tr><td align='right'><label>".__('Winner to', 'o-kompas')."</label></td><td><input id='nextmatchid' type='text' name='nextmatchid' value='$nextmatchid' /></td></tr>";
...
function ocm_comp_format_knockout_match_list_header( $html, $compid ) {
	...
	$names = array( __("Number #", 'o-kompas'), __("Winner to", 'o-kompas') );
