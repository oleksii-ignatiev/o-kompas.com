<?php 
    ...
    if ( $ocm_competition_export_hidden == 'yes' ) {
        ...
        header("Content-Disposition: attachment; filename=" . __('Matches', 'o-kompas') . ".csv");
...
<form name="ocm_competition_export_form" method="post" action="<?php echo admin_url('admin.php').'?page='.$_GET['page'].'&noheader=true'; ?>">
    <div class="metabox-holder ">
        <div class="postbox"> 
            <h3><?php _e('Export Competitions', 'o-kompas') ?></h3>
        </div>
    </div>
<?php
    echo apply_filters( 'ocm_competitions_export_selection', "" );
?>        
    <input type="submit" name="submit_competition_export" class="button-primary" value="<?php _e('Export Competitions to File', 'o-kompas')?>" />
