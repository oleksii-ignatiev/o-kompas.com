...
add_shortcode( 'ocm_match_data', 'ocm_sh_match_data' );
add_shortcode( 'ocm_match_current', 'ocm_sh_match_current');

function ocm_sh_match_data( $atts ) {
	...
	if ( $match_id != '' ) {
		...
		$html = ocm_sh_matches( $competition_id, $competitor_id, $self_link_ok, $order );
		// if competitor_id not specified, competition format may have different scheme to display matches
		$html = apply_filters( 'ocm_comp_display_all_matches', $html, $competition_id);		
	}
	
    return "$before \n" . "<p>" . $html . "</p>" . "$after \n";
} 
  
function ocm_sh_match( $match_id ) {
	...
	$compformat = apply_filters( 'ocm_competition_get_format_name', "", $competition_id );

	$resultstring = ( chop($result,":") == '' ? __('vs', 'sports-club-management') : apply_filters( 'ocm_match_display_result', $result, $competition_id ));
	$comp1string  = ocm_match_get_linked_competitor( apply_filters( 'ocm_comp_display_match_competitor_1', get_the_title( $compid_1 ), $match_id, array(), $competition_id )
													   , $compid_1, 0, true 
													   );
	$comp2string  = ocm_match_get_linked_competitor( apply_filters( 'ocm_comp_display_match_competitor_2', get_the_title( $compid_2 ), $match_id, array(), $competition_id )
													   , $compid_2, 0, true 
													   );
...
function ocm_sh_matches( $competition_id, $competitor_id, $self_link_ok, $order ) {
    ...
    if ( count( $matches ) == 0 ) {
		...
		$html .= "<div id=\"scm_match_meta\" class=\"header $compformat\">" . apply_filters( 'ocm_comp_display_match_meta_header', "", $competition_id) . "</div>\n";
        ...
        foreach ( $matches as $match ) {
			...
			$timeresultstring = ( chop($result,":") == '' ? $time : apply_filters( 'ocm_match_display_result', $result, $competition_id ) );
			$comp1string      = ocm_match_get_linked_competitor( apply_filters( 'ocm_comp_display_match_competitor_1', get_the_title( $compid_1 ), $match->ID, $matches, $competition_id )
															   , $compid_1, $competitor_id, $self_link_ok 
															   );
			$comp2string      = ocm_match_get_linked_competitor( apply_filters( 'ocm_comp_display_match_competitor_2', get_the_title( $compid_2 ), $match->ID, $matches, $competition_id )
															   , $compid_2, $competitor_id, $self_link_ok 
															   );
			
			$html .= "<div id=\"scm_match\" class=\"$compformat\">\n";
			$html .= "<div id=\"scm_match_meta\" class=\"$compformat\">" . apply_filters( 'ocm_comp_display_match_meta', "", $match->ID, $competition_id) . "</div>\n";
...
function ocm_match_get_linked_competitor( $title, $comp_id, $self, $self_link_ok ) {
...
function ocm_sh_match_current( $atts ) {
	...
	foreach ( $matches as $match ) {
		...
		$html .= "<div id=\"ocm_widget_match\">\n";
		$html .= "<div id=\"ocm_widget_competitor\" class=\"comp1 $compformat\">" . apply_filters( 'ocm_comp_display_match_competitor_1', get_the_title( $compid_1 ), $match->ID, $matches, $competitionid ) . "</div>\n";
		$html .= "<div id=\"ocm_widget_result\" class=\"$compformat\">" . ($match->post_content != "" ? "<a href=".get_post_permalink( $match->ID ).">".$dateresultstring."</a>" : $dateresultstring) . "</div>\n";
		$html .= "<div id=\"ocm_widget_competitor\" class=\"comp2 $compformat\">" . apply_filters( 'ocm_comp_display_match_competitor_2', get_the_title( $compid_2 ), $match->ID, $matches, $competitionid ) . "</div>\n";
		$html .= "</div>";
	}
	if ( count( $matches ) == 0 ) {
		$html .= $emptylist;
