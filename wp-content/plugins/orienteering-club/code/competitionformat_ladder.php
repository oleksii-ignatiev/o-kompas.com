...
define('OCM_CFMT_LADDER', 1004); 

include_once 'scores.php';

add_filter('ocm_competition_get_format_name', 'ocm_comp_format_ladder_get_format_name', 10, 2 );
add_filter('ocm_competition_get_formats', 'ocm_comp_format_ladder_get_format', 10, 2 );
add_filter('ocm_competition_display_format', 'ocm_comp_format_ladder_display_format', 10, 2 );
add_filter('ocm_competition_display_ranking', 'ocm_comp_format_ladder_display_ranking', 10, 4 );

function ocm_comp_format_ladder_get_format_name( $html, $compid ) {

    if ( get_post_meta( $compid, '_formatid', true) == OCM_CFMT_LADDER ) {
        $html = "ladder"; 
    }
...
function ocm_comp_format_ladder_get_format( $html, $id ) {
    $html .= "<option ";      
    $html .= ($id == OCM_CFMT_LADDER) ? "selected='selected' " : "";   
    $html .= "value=" . OCM_CFMT_LADDER . ">" . __('Ladder', 'sports-club-management') . "</option>\n";

    return $html; 
}

function ocm_comp_format_ladder_display_format( $html, $compid ) {
...
function ocm_comp_format_ladder_display_ranking( $html, $compid, $display_all, $display_header ) {
    ...
    $rankings = ocm_comp_format_ladder_get_ranking($compid);
...
function ocm_comp_format_ladder_get_ranking($compid) {
